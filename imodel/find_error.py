...
for i in range(train_iter):
    ...
    if (i + 1) % 200 == 0:
        train_accuracy = sess.run(accuracy, feed_dict={_X: batch[0], y: labels, keep_prob: 1.0})
...
for i in range(test_iter):
    preLabel = sess.run(predictions["classes"], feed_dict={_X: np.reshape(data_test[i], (-1,1600)), y: np.reshape(label_test[i], (-1, 4)), keep_prob: 1.0})
